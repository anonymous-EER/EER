...
int main() {
    ...
    */
    ...
    for(int pnum = 0; pnum < 31; pnum++) {
        ...
        std::ifstream file("generalized/model_saves/"+std::to_string(pnum)+"_100_0.json");
        //std::ifstream file("generalized/model_datas/Given/2/"+std::to_string(pnum)+"_100_0.json");
        if (!file.is_open()) {
        ...
        std::vector<std::vector<double>> x_preds_temp;
        std::vector<std::vector<double>> x_tests_temp;
        //std::vector<double> y_test;
        
        for (const auto& sample : j["x_preds"]) {
            std::vector<double> sample_vec;
            for (const auto& row : sample) {
                sample_vec.push_back(row);
            }
            x_preds_temp.push_back(sample_vec);
        }
       ...
       // std::cout << "x_preds 크기: " << x_preds.size() << " x "
        ...
        for (const auto& sample : j["x_preds"]) {
            ...
            x_tests_temp.push_back(sample_vec);
        }
       ...
       // std::cout << "y_test 크기: " << y_test.size() << " x "
        ...
        //std::ifstream file2("trained_saves/Given/"+std::to_string(pnum)+"_100_0_0.json");
        //std::ifstream file2("trained_saves/Wesad/"+std::to_string(pnum)+"_100_0.json");
        std::ifstream file2("generalized/model_saves/"+std::to_string(pnum)+"_100_0.json");
        if (!file2.is_open()) {
            std::cerr << "파일 열기 실패\n";
            return 1;
        }
        ...
        std::ifstream file3("trained_saves/Given/"+std::to_string(pnum)+"_100_1_0.json");
        //std::ifstream file3("trained_saves/Wesad/"+std::to_string(pnum)+"_100_1.json");
        if (!file3.is_open()) {
            std::cerr << "파일 열기 실패\n";
            return 1;
        }
        ...
        //Model model(100, 100,0.01,61,82,32);
        ...
        std::vector<HEaaN::Ciphertext> precom_w1;
        precompute_sigma_psi(eval,ctxt_w1,precom_w1,32,sigma_plain,tau_plain,phi_plain);
        ...
        std::vector<HEaaN::Ciphertext> precom_w2;
        precompute_sigma_psi(eval,ctxt_w2,precom_w2,1,sigma_plain,tau_plain,phi_plain);


        // 분할 수행 (120 x 84 단위로)
        auto chunks = split_matrix(x_tests_temp, 128);
        ...
        for(int i=0; i<chunks.size(); i++) {
            ...
            packed(chunks[i],chunks2[i],packed_x);
            ...
            //printMessage(packed_x,false,20,0);
            ...
            //timer.start("Inference Latency (for 128 datas) : ");
            model.inference(eval,btp,precom_w1,precom_w2,ctxt,ctxt_res,sigma_plain,tau_plain,phi_plain);
            //timer.end();
            ...
            sk.to(HEaaN::getCurrentCudaDevice());
            //std::cout << "debug\n";
            dec.decrypt(ctxt_res,sk,packed_y);
            //std::cout << "debug\n";
            ...
            for(int j=0; j<chunks[i].size(); j++) {
                ...
                pred1.push_back({y1[0][j]});
                pred2.push_back({y2[0][j]});
            }
        }
        ...
        for(int i=0; i<y_test.size(); i++) {
            ...
            if(round((pred1[i][0])) == y_test[i][0]) {
                acc1+=1.0;
            }

            if(round((pred2[i][0])) == y_test[i][0]) {
                acc2+=1.0;
            }

            //std::cout << i << " : " << pred1[i][0] << " / " << pred2[i][0] << '\n';
        }
        ...
        std::cout << std::to_string(pnum)<< "\'s acc : " << acc / y_test.size() << '\n';
        std::cout << std::to_string(pnum)<< "\'s acc1 : " << acc1 / y_test.size() << " , acc2 : " << acc2 / y_test.size() << '\n' ;
    ...
    std::cout << "Leave out f1-score : " << f1_mean/(double)f1s.size() << '\n';
